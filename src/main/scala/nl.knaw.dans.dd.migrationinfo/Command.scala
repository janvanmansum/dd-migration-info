...
object Command extends App with DebugEnhancedLogging {
  ...
  val app = new DdMigrationInfoApp(configuration)
...
//      case None => // handle command line without subcommands
  ...
  private def runAsService(app: DdMigrationInfoApp): Try[FeedBackMessage] = Try {
    val service = new DdMigrationInfoService(configuration.serverPort, Map(
      "/" -> new DdMigrationInfoServlet(app, configuration.version),
    ))
